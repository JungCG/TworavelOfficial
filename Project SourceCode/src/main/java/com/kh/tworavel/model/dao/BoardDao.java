...
public class BoardDao {
	...
	private SqlSession sqlSession;
	 ...
	 public List<Board> selectHotViewList(){ // 특정 페이지 단위의 게시글 조회
		 ...
		 public List<Board> searchList(String keyword,int startPage,int limit) { // 게시글 검색 조회
			 int startRow = (startPage-1)*limit;
			 RowBounds row = new RowBounds(startRow, limit);
			 return sqlSession.selectList("Board.selectSearch", keyword,row);
			 }
 ...
 public int selectLikeCount(int b_id) {
	...
	public String selectWriterBoard(int m_id) {
		return sqlSession.selectOne("Board.selectWriterBoard",m_id);
	}
}
